...
namespace Trace {
    ...
    __forceinline __host__ __device__ float3 computeBarycenter(const std::vector<float3>& vtx, const std::vector<int3>& idx, int objID) {
    ...
    public:
        ...
        virtual __host__ float3 computeNormal(int idx) const override {
        ...
        virtual bool hit(int idx, const Trace::Ray& ray, float t_max, float& root) const override {
            ...
            root = f * dot(edge2, q);
            if (root > 0.0001f && root < t_max) {  // Reduced threshold
                return true;
            }
        ...
        virtual dPrimitive* createDeviceVersion() const override {
            ...
            dPrimitive* d_triangle;
            cudaMalloc((void**) &d_triangle, sizeof(dPrimitive));
            ...
            cudaMemcpy(d_triangle, &temp, sizeof(dPrimitive), cudaMemcpyHostToDevice);
